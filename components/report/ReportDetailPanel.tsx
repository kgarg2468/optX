...
import { X, Send, Pin, Square } from "lucide-react";
...
export function ReportDetailPanel({
  ...
  const buildContext = () => {
    ...
    return `[Report Context]\n${pinnedItems.map((i) => `${i.type}: ${i.title} — ${i.aiDetail}`).join("\n")}`;
  };
        ...
        <TabsContent value="analysis" className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
          {analysisItems.length === 0 ? (
            ...
            analysisItems.map((item) => {
              ...
              return (
                <div key={item.id} className="rounded-lg border border-white/[0.08] bg-white/[0.03] p-4 space-y-3 relative group">
                  ...
                  <div className="pt-2 border-t border-white/[0.08]">
                    <MarkdownMessage content={item.aiDetail} className="text-muted-foreground" />
                  </div>
                </div>
              );
            })
          ...
          {pinnedItems.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-3 shrink-0">
              {pinnedItems.map((item) => (
                <div key={item.id} className="flex items-center gap-1 rounded-full px-2 py-[3px] text-[10px] font-medium border border-amber-500/30 bg-amber-500/10">
                  ...
                  <button onClick={() => onDismissPinned(item.id)} className="ml-0.5 text-amber-400/60 hover:text-amber-400 transition-colors">
                    <X className="h-3 w-3" />
                  </button>
                </div>
              ))}
          ...
          <div className="flex gap-2 shrink-0 pt-2 border-t border-white/[0.05]">
            ...
            {isStreaming ? (
              <Button
                size="sm"
                variant="outline"
                className="h-9 w-9 p-0 border-rose-500/30 text-rose-400 hover:bg-rose-500/10"
                onClick={cancel}
                title="Stop"
              >
